...
def test_failed_session_start_reports_unavailable_without_retrying() -> None:
    FakeSession.start_error = PermissionError("ETW denied")
    collector = WindowsProcessNetworkCollector(
        session_factory=FakeSession,
        clock=FakeClock([1.0]),
    )
    ...
    assert isinstance(collector.last_error, PermissionError)
    assert collector.available is False


def test_close_stops_session_and_is_idempotent() -> None:
    collector = WindowsProcessNetworkCollector(
        session_factory=FakeSession,
        clock=FakeClock([1.0]),
    )
