...
物理桌面上，用户已确认微型窗可见，并观察到“后台”和“状态未知”；初次测试报告浏览器获得焦点且有正常速率时仍未显示“前台”。随后定位并修复了每次快照清空 Presence 缓存的问题：相同成员身份的快照保留结果，application key 或 `(pid, create_time)` 变化则失效，直接读取 frame 也检查身份上下文。新增回归测试已包含在上述结果中。2026-09-22 重启最新版后，用户明确确认固定关注浏览器并切换焦点时已正常完成“前台→后台→前台”；这项结果来自用户物理观察，其他桌面操作不因此视为通过。

用户随后确认卡片打开/固定关注、Compact、Detailed（含本次监控累计）、托盘恢复和从托盘退出均正常。三档系统 DPI 的真实桌面切换、拖动误触和多显示器行为尚未在本轮单独验证；自动布局测试不能替代这些观察。当前分支的 CI / ETW 结果以推送后精确提交对应的 Actions run 为准。
